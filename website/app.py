...
app_storymap_dir = os.path.join(settings.PROJECT_ROOT, 'storymap')
app_timelines_dir = os.path.join(settings.PROJECT_ROOT, 'timelines')
app_baidumap_dir = os.path.join(settings.PROJECT_ROOT, 'baidumap')
...
@app.route('/storymap/<path:path>')
def catch_storymap(path):
    """
    Serve /storymap/... urls from the storymap directory
    """
    return send_from_directory(app_storymap_dir, path)

@app.route('/timelines/<path:path>')
def catch_build(path):
    """
    Serve /timelines/... urls from the timelines directory
    """
    return send_from_directory(app_timelines_dir, path)

@app.route('/baidumap/<path:path>')
def catch_baidumap(path):
    """
    Serve /baidumap/... urls from the baidumap directory
    """
    return send_from_directory(app_baidumap_dir, path)

@app.route('/locale/<path:path>')
def catch_locale(path):
    """
    Serve /locale/... urls from the baidumap directory
    """
    return send_from_directory(locale_dir, path)
